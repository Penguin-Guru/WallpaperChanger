...
Bash users (with Readline) may enable rudimentary auto-completion by sourcing (`source`) the files in "[auto-completion/bash/](auto-completion/bash/)" or copying them to a directory where such files are automatically loaded. Note that auto-completion for the build script may have some unintended effects related to path scoping ([#13](https://github.com/Penguin-Guru/WallpaperChanger/issues/13)).
